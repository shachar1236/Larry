...
public:
    ...
    {
        types_bitmap.ForEachType(
            [&](TypesBitmap curr) { components[curr] = UnknownTypeVector(type_manager->GetTypeSize(curr)); });
    }

    bool operator==(const Archetype& other) { return this->types_bitmap == other.types_bitmap; }

    TypesBitmap GetTypesBitmap() { return types_bitmap; }

    int GetSize() { return entitys.size(); }
    ...
    void SetComponents(int index, const TypeQueue& types, AnyQueue& resultQueue)
    {
        ...
        for (auto& type : types.elements) {
            resultQueue.elements.push_back(
                { components[type_manager->GetTypeBitmap(type)].GetRawByIndex(index), type });
        }
    }
    ...
    int PopEntityFromOtherArchetype(Entity entity, Archetype& other, int entity_index_in_other_archtype)
    {
        ...
        intesecting.ForEachType(
            [&](TypesBitmap type) { other.components[type].Copy(old_index, components[type].GetRawByIndex(index)); });
    ...
    template <typename F>
    void CallFunctionWithComponents(const TypeQueue& type_queue, AnyQueue& system_components_queue,
        TypesBitmap singeltons_types, std::unordered_map<TypesBitmap, byte*>& singeltons, const F& callback, bool* stop)
    {
            ...
            if (entitys[i].alive) {
                ...
                for (auto& component_type : type_queue.elements) {
                    ...
                    if (singeltons_types.Intersect(t)) {
                        ...
                        system_components_queue.elements.push_back(
                            ECS_Any { components[t].GetRawByIndex(i), component_type });
                    }
                }
